...
{
    public partial class FrmListaDepositosSinCuenta : Form
    {
        public FrmListaDepositosSinCuenta()
        {
            InitializeComponent();
        }
        ...
        private List<OperacionRealizada> ListarOperacionesRealizadas()
        {
            ...
            {
                if (operacion is DepositoSinCuenta)
                {
                    var depositoSinCuenta = (DepositoSinCuenta)operacion;
                    encontradas.Add(new OperacionRealizada
                    {
                        Fecha = depositoSinCuenta.Fecha,
                        Moneda = depositoSinCuenta.CuentaDestino.NombreTipoMoneda,
                        Monto = depositoSinCuenta.Monto,
                        Tipo = "Depósito sin cuenta"
                    });
                }
            }
        ...
        private SLDocument CrearDocumentoExcel(DataTable tablaDatos)
        {
            ...
            documento.SetCellValue("B3", $"LISTA DE DEPÓSITOS SIN CUENTA DEL {DtpInicio.Value.Date:dd/MM/yyyy} AL {DtpFin.Value.Date:dd/MM/yyyy}");
